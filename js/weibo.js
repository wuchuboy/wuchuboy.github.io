fetch('https://wb.bore.vip/api').then(data=>data.json()).then(data=>{
    ...
    for (let item of data) {
        var number = (item.num/10000).toFixed(1); // 改为万为单位
        html += '<div class="weibo-list-item"><div class="weibo-hotness ' + hotness[(item.hot || '热')] + '">' + (item.hot || '热') + '</div>' + '<span class="weibo-title"><a title="' + item.title + '"href="' + item.url + '" target="_blank" rel="external nofollow noreferrer">' + item.title + '</a></span>' + '<div class="weibo-num"><span>' + number + 'W</span></div></div>'
    }
    html += '</div>'
